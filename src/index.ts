...
function createUnderscore(guessWord: string) {
...
let checkLetters: (a: string, b: string, c: string[], d: KeyboardEvent) => void;
...
checkLetters = (guessWord, character, correctWord, event) => {
  ...
  if (code >= 97 && code <= 122) {
    if (guessWord.includes(character)) {
      ...
      const wrongSound = document.getElementById(
        "wrongSound"
      ) as HTMLAudioElement;
...
function checkCorrectWord(guessWord: string, correctWord: string[]) {
